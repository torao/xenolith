...
| [`xylograph-exslt`](crates/xylograph-exslt) | Phase 6.5e | EXSLT extension functions, one feature per module: `math` (minimum, maximum, powers, logarithms, trigonometry), `sets` (difference, intersection, distinct and the rest), `strings` (tokenize, split, padding, alignment, URI escaping), `dates` (reading an ISO 8601 date or time apart) and `common` (`object-type()`, `node-set()`). Nothing is built into the engine — these are registered the way any caller's functions are. `functions` and `regular-expressions` are still to come |
