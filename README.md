...
| [`xylograph-exslt`](crates/xylograph-exslt) | Phase 6.5a | EXSLT extension functions, one feature per module: `math` (minimum, maximum, powers, logarithms, trigonometry), `sets` (difference, intersection, distinct and the rest) and `common`'s `object-type()`. Nothing is built into the engine — these are registered the way any caller's functions are |
