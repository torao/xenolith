...
- `key()` は構文として受理するがキー表が未実装なので何にもマッチしない（5c で解消）
- **成果物**: `xylograph-xslt` クレート、`ErrorKind::Xslt`、統合テスト 13 + doctest
- **完了条件**: 各種パターンのマッチングと既定優先度が通り、部分集合外が拒否される

**5b. スタイルシートのモデルとコンパイル**
- スタイルシート文書 → `Stylesheet`（テンプレート規則、トップレベル変数・パラメータ）
- `xsl:import` / `xsl:include`、インポート優先順位、テンプレート規則の衝突解決（優先順位 → 文書順）

**5c. 実行エンジン**
- `apply-templates` / `call-template` / `for-each` / `if` / `choose` / `value-of` / `variable` / `param`、モード、組み込み規則
- リテラル結果要素、属性値テンプレート、RTF、`key()`
