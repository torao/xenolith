...
//! cargo bench -p xylograph -- xslt      # one group
//! ```
//!
...
use xylograph::xdm::DomModel;
use xylograph::xpath::XPath;
use xylograph::xslt::{Stylesheet, Transform};
...
fn parsing(c: &mut Criterion) {
  ...
  for &records in SIZES {
    ...
    group.bench_with_input(BenchmarkId::new("events", records), &xml, |b, xml| {
      b.iter(|| {
        let mut reader = xylograph::parser::Reader::new(xml.as_bytes());
