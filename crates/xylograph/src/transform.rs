...
impl<'a> Source<'a> {
  ...
  fn system_id(&self) -> String {
    self.system_id.clone().unwrap_or_else(|| "urn:xylograph:unnamed-source".to_owned())
  }
}
...
#[cfg(feature = "exslt")]
fn extension_functions<M: xylograph_xdm::Model>(trees: &Rc<dyn DocumentSource<M::Node>>) -> Functions<M> {
  xylograph_exslt::register_with(Functions::new(), trees)
}
...
impl Transformed {
  ...
  fn copied(document: &Document) -> Self {
    let written = match document.document_element() {
      Some(root) => xylograph_serialize::Serializer::new().to_string(document, root),
      None => String::new(),
    };
