...
| Java | Here |
|---|---|
| `javax.xml.parsers.DocumentBuilder` | [`dom::build::parse`](xylograph_dom::build::parse) |
| `org.w3c.dom.Document`, `Node`, `Element` | [`dom::Document`](xylograph_dom::Document) and a `Copy` [`NodeId`](xylograph_dom::NodeId) |
| `org.w3c.dom.DOMException` | [`dom::DomException`](xylograph_dom::DomException) |
| `javax.xml.stream.XMLStreamReader` (StAX) | [`parser::Reader`](xylograph_parser::Reader) |
| `org.xml.sax.ContentHandler` + `SAXParser` | [`parser::sax::Handler`](xylograph_parser::sax::Handler) + [`sax::drive`](xylograph_parser::sax::drive) |
| `org.xml.sax.EntityResolver` | [`parser::resolve::UriResolver`](xylograph_parser::resolve::UriResolver) |
| `setValidating(true)`, `javax.xml.validation` | [`validate::validate`](xylograph_validate::validate), [`Validator`](xylograph_validate::Validator) |
| `javax.xml.stream.XMLStreamWriter` | [`serialize::XmlWriter`](xylograph_serialize::XmlWriter) |
| `LSSerializer` / `Transformer` used to print a tree | [`serialize::Serializer`](xylograph_serialize::Serializer) |
| `XPathFactory.newInstance().newXPath()` | [`xpath::XPath::new`](xylograph_xpath::XPath::new) |
| `NamespaceContext` | [`XPath::with_namespace`](xylograph_xpath::XPath::with_namespace) |
| `XPathVariableResolver` | [`xpath::Variables`](xylograph_xpath::Variables) |
| `XPathFunctionResolver` / `XPathFunction` | [`xpath::Functions`](xylograph_xpath::Functions) / [`Function`](xylograph_xpath::Function) |
...
| `URIResolver` | [`transform::Resolver`](crate::transform::Resolver) (which is [`xslt::Loader`](xylograph_xslt::Loader)) |
...
| `xalan` / `xsltproc` on the command line | the `xylo` binary |
...
```rust
use xylograph::parser::sax::{Handler, drive};
use xylograph::parser::{Parser, Reader};
...
[data model](xylograph_xdm) — the seven node kinds, merged text, synthesized namespace nodes —
which `DomModel` presents over a borrowed `Document` without changing it. And there is no
`XPathConstants`: the [`Value`](xylograph_xpath::Value) that comes back *is* one of the four
...
```rust
use xylograph::validate::validate;
...
[`Report`](xylograph_validate::Report) and the document is read to the end. A document with no
...
```rust
use xylograph::serialize::XmlWriter;
...
[`ErrorListener`](xylograph_validate::ErrorListener) decides whether to carry on after a
...
[`UriResolver`](xylograph_parser::resolve::UriResolver) fetches nothing, and so does a
