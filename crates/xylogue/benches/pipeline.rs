...
//! cargo bench -p xylogue -- xslt      # one group
//! ```
//!
...
use xylogue::xdm::DomModel;
use xylogue::xpath::XPath;
use xylogue::xslt::{Stylesheet, Transform};
...
fn parsing(c: &mut Criterion) {
  ...
  for &records in SIZES {
    ...
    group.bench_with_input(BenchmarkId::new("events", records), &xml, |b, xml| {
      b.iter(|| {
        let mut reader = xylogue::parser::Reader::new(xml.as_bytes());
