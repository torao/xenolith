...
//!
//! ```text
//! cargo test -p xylogue --test behaviour -- --nocapture
//! ```
//!
...
use xylogue::dom::{Document, build};
...
fn ordered_across_documents() -> String {
  use std::rc::Rc;
  use xylogue::xdm::Documents;
  use xylogue::xslt::{LoadedDocuments, Loader};
  ...
  let stylesheet = xylogue::xslt::Stylesheet::compile(
  ...
  let result = xylogue::xslt::Transform::new()
    .run_with_documents(&stylesheet, &model, model.root_node(), xylogue::xpath::Functions::new(), available)
    .expect("transforms");
...
fn formatted(expression: &str) -> String {
  ...
  let stylesheet = xylogue::xslt::Stylesheet::compile(source.as_bytes(), "file:///s.xsl").expect("compiles");
  let doc = build::parse("<r/>".as_bytes()).expect("well-formed");
  let model = DomModel::new(&doc);
  xylogue::xslt::transform(&stylesheet, &model, model.root_node()).expect("transforms").text()
}
...
fn sorted(xml: &str, sort: &str) -> String {
  ...
  let doc = build::parse(xml.as_bytes()).expect("well-formed");
  let model = DomModel::new(&doc);
  let result = xylogue::xslt::transform(&stylesheet, &model, model.root_node()).expect("transforms");
  result.text().trim_end().to_owned()
}
...
fn report_of_behaviour_the_specifications_leave_open() {
  println!("\n============================================================");
  println!(" xylogue — behaviour where the specifications do not say");
  println!("============================================================\n");
...
fn dependent_on_the_build() -> Vec<Item> {
  ...
  let available: Vec<&str> = encodings.into_iter().filter(|label| xylogue::encoding::lookup(label).is_some()).collect();
  ...
  vec![
    ...
    .observe("language-aware collation (feature icu)", xylogue::xslt::language_aware_collation().to_string()),
    item(
      "XSLT 1.0 §10",
    ...
    .observe("this build", if xylogue::xslt::language_aware_collation() { "has it" } else { "does not" })
