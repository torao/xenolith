...
//!
//! xenolith does not *implement* the [SAX API](http://www.saxproject.org/ "SAX: Simple API for XML"); this is
...
//! use xenolith_parser::sax::{EndElementEvent, Handler, StartElementEvent, parse};
//!
//! #[derive(Default)]
...
//! use xenolith_parser::sax::{Handler, StartElementEvent, parse};
//!
...
//! SAX's line between a recoverable violation ([`Severity::Error`](xenolith_core::Severity), a validity error) and a
//! fatal one ([`Severity::Fatal`](xenolith_core::Severity)). A handler's own error comes back wrapped in
//! [`Error::SaxHandler`].
//!
//! ```
//! use std::convert::Infallible;
//!
//! use xenolith_parser::Reader;
//! use xenolith_parser::sax::{Handler, parse};
...
//! use xenolith_parser::sax::{DoctypeEvent, Handler, parse};
//!
//! #[derive(Default)]
