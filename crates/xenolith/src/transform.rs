...
use xenolith_xdm::{Documents, DomModel, DomNode};
use xenolith_xpath::Functions;
use xenolith_xslt::{DocumentSource, LoadedDocuments, Loader, NoLoader, ResultSink, Stylesheet, Transform, TreeSpace};

/// The method `xsl:output` asked the result to be written by.
pub use xenolith_xslt::OutputMethod;
...
#[cfg(feature = "exslt")]
fn extension_functions<M: xenolith_xdm::Model>(trees: &Rc<dyn DocumentSource<M::Node>>) -> Functions<M> {
  xenolith_exslt::register_with(Functions::new(), trees)
}
...
impl Transformed {
  ...
  fn copied(document: &Document) -> Self {
    let written = match document.document_element() {
      Some(root) => xenolith_serialize::Serializer::new().to_string(document, root),
      None => String::new(),
    };
