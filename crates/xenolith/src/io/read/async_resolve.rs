...
//!
//! The counterparts of [`UriResolver`](crate::io::resolve::UriResolver) for a driver that reads with `async fn`.
...
/// asynchronous runtime in the [`AsyncUriResolver`] signature. The interface of this type consists solely of the
...
impl AsyncEntityReader {
  ...
  /// Reads the entity to the end and stores it in `out`. The driver uses this for the DTD-side kinds (the external
  /// subset and parameter entities), which are read whole.
  ///
...
/// [`UriResolver`](crate::io::resolve::UriResolver).
///
/// This trait and the blocking [`UriResolver`](crate::io::resolve::UriResolver) serve the same purpose but are
/// intentionally separated. This lets asynchronous drivers avoid blocking the runtime thread to retrieve entities,
/// and blocking drivers avoid hosting the runtime. See [`UriResolver`](crate::io::resolve::UriResolver) for the
/// resolution specification. This explains only the difference in the asynchronous implementation.
///
/// This returns a non-blocking input stream [`AsyncEntityReader`]. You can construct this from an asynchronous reader
/// using [`AsyncEntityReader::from_async_read`] (or [`from_tokio`](AsyncEntityReader::from_tokio) under the `tokio`
/// feature). Neither resolving the request nor reading the entity blocks the runtime thread because the driver uses
/// `.await` to process that input stream.
///
#[cfg(feature = "async")]
