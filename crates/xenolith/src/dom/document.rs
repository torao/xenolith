//! The document: the [arena](crate::dom#arena) that owns every node and the operations over it.
//!
...
//! navigation and value accessors, or through a [`NodeRef`](crate::dom::NodeRef) for chained reads.
...
use crate::chars;
use crate::name::{NameId, NamePool, QName, XML_NS_URI, XML_PREFIX, XMLNS_NS_URI, XMLNS_PREFIX};

use crate::dom::collection::{NamedNodeMap, NodeList, Query};
use crate::dom::exception::{DomException, ExceptionCode, Result};
use crate::dom::node::{AttrData, ElementData, NodeData, NodeId, NodeSlot, NodeType};
use crate::dom::noderef::NodeRef;
...
/// rather than reporting it. Use [`owns`](Self::owns) to test a handle whose origin is in doubt.
///
/// # Examples
///
/// ```
...
impl Document {
  ...
  /// Creates an element in a namespace, from a namespace name and a qualified name. The new node is not yet attached.
  ///
  /// # Errors
  ///
  /// [`ExceptionCode::INVALID_CHARACTER_ERR`] if `qualified_name` is not a legal name;
  ...
  /// A document built by [`DomBuilder`](crate::dom::build::DomBuilder) already has the attributes its DTD declares `ID`
  /// marked, and its `xml:id` attributes unless the builder was told not to.
  ///
  /// # Cost
  ///
  ...
  ///
  /// `node` serves as the starting point for the search, but `node` itself is not included in the list. Although the
  /// DOM specification defines this method for both `Document` and `Element`, it is defined here as a single method.
  /// Specifying [`document_node`](Self::document_node) for `id` causes the entire document to be searched, mirroring
  /// the behavior of the `Document` object; conversely,
  /// [`NodeRef::get_elements_by_tag_name`](crate::dom::NodeRef::get_elements_by_tag_name) behaves similarly to the
  /// method on `Element`.
  #[must_use]
  pub fn get_elements_by_tag_name(&self, node: NodeId, name: &str) -> NodeList<'_> {
    NodeList::new(self, Query::by_tag_name(node, name))
  }
  ...
  ///
  /// As with [`get_elements_by_tag_name`](Self::get_elements_by_tag_name), `node` serves as the starting point for the
  /// search.
  #[must_use]
  pub fn get_elements_by_tag_name_ns(&self, node: NodeId, namespace: Option<&str>, local: &str) -> NodeList<'_> {
    NodeList::new(self, Query::by_tag_name_ns(node, namespace, local))
  }
  ...
  ///
  /// The node keeps its slot in the arena. Only dropping the document reclaims it.
  ///
  /// # Errors
  ///
...
fn check_qname_namespace(namespace: Option<&str>, qualified_name: &str, is_attribute: bool) -> Result<()> {
  ...
  if prefix == Some(XML_PREFIX) && namespace != Some(XML_NS_URI) {
    return namespace_error("the prefix \"xml\" may only name the XML namespace");
  }
  if is_attribute {
    let is_xmlns = qualified_name == XMLNS_PREFIX || prefix == Some(XMLNS_PREFIX);
