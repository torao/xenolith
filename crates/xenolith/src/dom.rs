//! A Document Object Mode (DOM) implementation with W3C-compliant API, built as an arena of nodes.
//!
//! # The arena structure
//!
//! <a id="arena"></a>
//! The tree is an **arena**: one [`Document`] owns every node, and they are identified by a small `Copy`-type
//! [`NodeId`]. A node is then cheap to hold and to compare. Unlike typical graphs of reference-counted objects
...
//!
//! This module is designed for use cases where a tree is built and read, and the entire document is discarded once the
...
//!
//! The traversal processing in this module does not use recursion based on depth. Recursion consumes one call stack
...
//! ```
//!
//! # Building from events and emitting as events
//!
//! Instead of the application independently building or traversing a tree, you can also combine modular processing
//! steps applied to an event stream. [`build::DomBuilder`] constructs a tree from a stream of events, while
//! [`DomSource`] traverses that tree and converts it into a sequence of events. Both operate solely using the
//! [`event`](crate::event) vocabulary, and the tree itself remains independent of the parser.
//!
//! # Specifications
//!
//! This module is implemented from the following documents, at the URLs with the date:
//!
...
//!   of reference-counted objects rather than an arena, this module redesigns it rather than imitating it (see
//!   [the arena](crate::dom#arena)).
