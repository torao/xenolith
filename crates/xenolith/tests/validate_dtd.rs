...
#[test]
fn a_namespace_declaration_is_an_attribute_the_dtd_has_to_declare() {
  let dtd = "<!DOCTYPE a [<!ELEMENT a (p:b)><!ELEMENT p:b EMPTY>\
             <!ATTLIST a xmlns:p CDATA #FIXED 'urn:p'>]>";

  assert!(
    is_valid(&format!("{dtd}<a xmlns:p='urn:p'><p:b/></a>")),
    "{:?}",
    errors(&format!("{dtd}<a xmlns:p='urn:p'><p:b/></a>"))
  );
  // Declared #FIXED, and given another namespace name.
  let other = format!("{dtd}<a xmlns:p='urn:other'><p:b/></a>");
  assert!(errors(&other).iter().any(|e| e.contains("xmlns:p") && e.contains("FIXED")), "{:?}", errors(&other));
  // Not declared at all.
  let undeclared = "<!DOCTYPE a [<!ELEMENT a EMPTY>]><a xmlns='urn:d'/>";
  assert!(errors(undeclared).iter().any(|e| e.contains("\"xmlns\" is not declared")), "{:?}", errors(undeclared));
}
