...
//! XMLCONF=xmlconf cargo test -p xenolith --test parser_conformance
//! ```
//!
...
///
/// The catalogue is read with xenolith itself, which exercises the reader on a few hundred
...
fn cases(root: &Path) -> Vec<(String, PathBuf, String)> {
  ...
  for catalogue in catalogues {
    ...
    let mut reader = StreamSource::new(std::io::BufReader::new(file));
