...
//!
//! ```text
//! cargo test -p xenolith --test behaviour -- --nocapture
//! ```
//!
...
use xenolith::dom::{Document, build};
...
fn ordered_across_documents() -> String {
  use std::rc::Rc;
  use xenolith::xdm::Documents;
  use xenolith::xslt::{LoadedDocuments, Loader};

  /// A loader serving two documents, each with two items.
  struct Two;
  impl Loader for Two {
    fn load(&mut self, uri: &str) -> xenolith::Result<Vec<u8>> {
  ...
  let stylesheet = xenolith::xslt::Stylesheet::compile(
  ...
  let result = xenolith::xslt::Transform::new()
    .run_with_documents(&stylesheet, &model, model.root_node(), xenolith::xpath::Functions::new(), available)
    .expect("transforms");
...
fn formatted(expression: &str) -> String {
  ...
  let stylesheet = xenolith::xslt::Stylesheet::compile(source.as_bytes(), "file:///s.xsl").expect("compiles");
  let doc = build::parse("<r/>".as_bytes()).expect("well-formed");
  let model = DomModel::new(&doc);
  xenolith::xslt::transform(&stylesheet, &model, model.root_node()).expect("transforms").text()
}
...
fn sorted(xml: &str, sort: &str) -> String {
  ...
  let doc = build::parse(xml.as_bytes()).expect("well-formed");
  let model = DomModel::new(&doc);
  let result = xenolith::xslt::transform(&stylesheet, &model, model.root_node()).expect("transforms");
  result.text().trim_end().to_owned()
}
...
fn report_of_behaviour_the_specifications_leave_open() {
  println!("\n============================================================");
  println!(" xenolith — behaviour where the specifications do not say");
  println!("============================================================\n");
...
fn chosen_by_this_implementation() -> Vec<Item> {
  ...
  vec![
    ...
    .observe("max nested entity depth", limit_value(xenolith::parser::Limits::default().max_depth))
    .observe("max expansions", limit_value(xenolith::parser::Limits::default().max_expansions))
    .observe("max expanded characters", limit_value(xenolith::parser::Limits::default().max_expansion_chars))
    .observe("max element nesting depth", limit_value(xenolith::parser::Limits::default().max_element_depth))
...
fn dependent_on_the_build() -> Vec<Item> {
  ...
  let available: Vec<&str> =
    encodings.into_iter().filter(|label| xenolith::encoding::lookup(label).is_some()).collect();
  ...
  vec![
    ...
    .observe("language-aware collation (feature icu)", xenolith::xslt::language_aware_collation().to_string()),
    item(
      "XSLT 1.0 §10",
    ...
    .observe("this build", if xenolith::xslt::language_aware_collation() { "has it" } else { "does not" })
