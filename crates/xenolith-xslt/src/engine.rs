...
impl Transform {
  ...
  /// [`Documents`](xenolith_xdm::Documents) handle with the model.
  ///
  /// # Errors
  ///
...
impl<M: Model> Engine<'_, M> {
  ...
  fn run_node(&mut self, module: usize, node: NodeId, focus: Focus<M::Node>) -> Result<()> {
    ...
    match document.node_type(node) {
      NodeType::TEXT_NODE | NodeType::CDATA_SECTION_NODE => {
        ...
        if !text.chars().all(xenolith_core::chars::is_whitespace) {
          self.append_text(&text);
        }
        Ok(())
      }
  ...
  fn exsl_document(&mut self, module: usize, element: NodeId, focus: Focus<M::Node>) -> Result<()> {
    ...
    let href = xenolith_core::uri::resolve(&base, &href)?;
  ...
  fn evaluate(
    ...
    xenolith_xpath::evaluate_in(&parsed, &context)
  }
  ...
  fn expression(&mut self, expression: &str) -> Result<Expr> {
    ...
    let parsed = xenolith_xpath::parse(expression)?;
  ...
  fn is_stripped(&self, node: M::Node) -> bool {
    ...
    if !self.model.string_value(node).chars().all(xenolith_core::chars::is_whitespace) {
      return false;
    }
    ...
    while let Some(node) = current {
      for attribute in self.model.attributes(node) {
        let is_xml_space = self.model.expanded_name(attribute).is_some_and(|name| {
          name.local == "space" && name.namespace.as_deref() == Some(xenolith_core::name::XML_NS_URI)
        });
        if is_xml_space {
