...
//! [XPath parser](xenolith_xpath::parse) and then checked against the subset, rather than given
//! a second grammar that could drift away from the first.

use xenolith_core::error::{Error, Result};
use xenolith_xdm::Model;
use xenolith_xpath::{Axis, BinaryOp, Expr, NameTest, Namespaces, NodeTest, Path, PathStart, Step, Variables};
...
impl Pattern {
  ...
  /// [`Error::Xslt`](xenolith_core::Error::Xslt) if it is a path but not one a pattern
  /// may be — an axis other than `child` or `attribute`, or an expression other than `id()` or
  /// `key()` at its head.
  pub fn compile(pattern: &str) -> Result<Self> {
    let expr = xenolith_xpath::parse(pattern)?;
...
fn step_matches<M: Model>(
  ...
  let selected = xenolith_xpath::evaluate_step(&full, model, parent, namespaces, variables)?;
...
const fn principal_kind(axis: Axis) -> xenolith_xdm::NodeKind {
  match axis {
    Axis::Attribute => xenolith_xdm::NodeKind::Attribute,
    _ => xenolith_xdm::NodeKind::Element,
  }
}
