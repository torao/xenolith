...
impl ResultSink for Collected {
  fn write(&mut self, href: &str, bytes: &[u8]) -> xenolith_core::Result<()> {
...
impl ResultSink for Refuses {
  fn write(&mut self, _href: &str, _bytes: &[u8]) -> xenolith_core::Result<()> {
    Err(xenolith_core::Error::io("this sink writes nothing".to_owned()))
  }
}
