...
impl EntityRequest {
  ...
  pub fn resolved_uri(&self) -> Option<String> {
    match &self.base_uri {
      Some(base) => xylogue_core::uri::resolve(base, &self.system_id).ok(),
...
pub trait UriResolver {
  ...
  /// failures) in [`Error::resolver`](xylogue_core::Error::resolver). This ensures that the error’s source is
  /// preserved, allowing the caller to restore the error by down-casting.
  ///
  fn resolve(
    &mut self,
    request: &EntityRequest,
  ) -> xylogue_core::error::Result<Option<Box<dyn std::io::Read + 'static>>>;
}
...
pub trait AsyncUriResolver {
  ...
  /// or network failure, say) with [`Error::resolver`](xylogue_core::Error::resolver), which keeps it as the error's
  /// source so a caller can recover it by downcasting.
  fn resolve(
    &mut self,
    request: &EntityRequest,
  ) -> impl std::future::Future<Output = xylogue_core::error::Result<Option<AsyncEntityReader>>> + Send;
}
