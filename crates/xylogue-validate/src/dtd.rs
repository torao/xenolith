...
use xylogue_core::error::Location;
use xylogue_core::name::{NameId, NamePool, QName};
use xylogue_parser::AttributeRef;
use xylogue_parser::dtd::{AttDef, AttType, ContentSpec, DefaultDecl, Dtd, GeneralEntity};
...
impl DtdValidator {
  ...
  fn default_value_is_valid(&self, att_type: &AttType, value: &str, pool: &NamePool) -> bool {
    ...
    match att_type {
      AttType::Cdata => true,
      AttType::Id | AttType::IdRef => xylogue_core::chars::is_name(value),
      AttType::IdRefs | AttType::Entities => tokens().all(xylogue_core::chars::is_name),
      AttType::Entity => {
        matches!(pool.get(value).and_then(|id| self.dtd.general_entity(id)), Some(GeneralEntity::Unparsed { .. }))
      }
      AttType::Nmtoken => xylogue_core::chars::is_nmtoken(value),
      AttType::Nmtokens => tokens().all(xylogue_core::chars::is_nmtoken),
  ...
  fn check_attribute_value(
    ...
    match att_type {
      ...
      AttType::Id => {
        if !xylogue_core::chars::is_name(value) {
      ...
      AttType::IdRefs => {
        ...
        for token in tokens {
          if !xylogue_core::chars::is_name(token) {
      ...
      AttType::Nmtoken => {
        if !xylogue_core::chars::is_nmtoken(value) {
      ...
      AttType::Nmtokens => {
        ...
        for token in tokens {
          if !xylogue_core::chars::is_nmtoken(token) {
