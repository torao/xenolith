...
fn every_document_parses_or_is_refused() {
  replay("documents", xylogue_fuzz::parse_document);
  replay("documents", xylogue_fuzz::parse_document_in_pieces);
  replay("documents", xylogue_fuzz::validate_document);
}

#[test]
fn every_document_that_builds_a_tree_can_be_written_and_read_back() {
  replay("documents", xylogue_fuzz::build_and_serialize);
}
...
fn every_stylesheet_compiles_or_is_refused() {
  replay("stylesheets", xylogue_fuzz::transform);
}
...
fn the_properties_survive_what_is_not_xml_at_all() {
  ...
  for input in [
    ...
    xylogue_fuzz::parse_document(input);
    xylogue_fuzz::parse_document_in_pieces(input);
    xylogue_fuzz::validate_document(input);
    xylogue_fuzz::build_and_serialize(input);
    xylogue_fuzz::transform(input);
