//! The properties xylogue is fuzzed against.
//!
...
pub fn validate_document(data: &[u8]) {
  if let Ok(report) = xylogue_validate::validate(data) {
...
pub fn build_and_serialize(data: &[u8]) {
  ...
  let written = xylogue_serialize::Serializer::new().to_string(&document, root);
  ...
  let again = xylogue_serialize::Serializer::new().to_string(&reread, reread_root);
