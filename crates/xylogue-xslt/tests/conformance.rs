...
use xylogue_xslt::{LoadedDocuments, Loader, OutputMethod, Stylesheet, Transform};
...
///
/// The catalogue is read with xylogue itself, which exercises the DOM builder on a megabyte of
...
fn catalogued_cases(root: &Path) -> Vec<Case> {
  ...
  let mut cases = Vec::new();
  let mut stack: Vec<(xylogue_dom::NodeId, String)> = Vec::new();
...
/// Reads one `test-case`.
fn read_case(document: &xylogue_dom::Document, element: xylogue_dom::NodeId, root: &Path, major: &str) -> Option<Case> {
...
impl UriResolver for Files {
  ...
  fn resolve(&mut self, request: &EntityRequest) -> xylogue_core::error::Result<Option<Vec<u8>>> {
...
fn normalize_indented(xml: &str, indented: bool) -> Option<String> {
  ...
  for child in document.children(document.root()) {
    ...
    if document.node_type(child) == xylogue_dom::NodeType::Text {
      continue;
    }
...
fn parse_loosely(xml: &str) -> Option<xylogue_dom::Document> {
  ...
  build::parse(format!("<xylogue-wrapper>{body}</xylogue-wrapper>").as_bytes()).ok()
}

/// Writes one node in the canonical form described on [`normalize`].
fn canonical(document: &xylogue_dom::Document, node: xylogue_dom::NodeId, indented: bool, into: &mut String) {
  use std::fmt::Write as _;
  match document.node_type(node) {
    xylogue_dom::NodeType::Element => {
      ...
      let among_elements =
        indented && document.children(node).any(|child| document.node_type(child) == xylogue_dom::NodeType::Element);
    ...
    xylogue_dom::NodeType::Text | xylogue_dom::NodeType::CdataSection => {
    ...
    xylogue_dom::NodeType::Comment => {
      let _ = write!(into, "<!--{}-->", document.node_value(node).unwrap_or_default());
    }
    xylogue_dom::NodeType::ProcessingInstruction => {
...
fn is_only_whitespace(document: &xylogue_dom::Document, node: xylogue_dom::NodeId) -> bool {
  document.node_type(node) == xylogue_dom::NodeType::Text
...
fn expanded(document: &xylogue_dom::Document, node: xylogue_dom::NodeId) -> String {
...
fn is_namespace_declaration(document: &xylogue_dom::Document, attribute: xylogue_dom::NodeId) -> bool {
...
fn the_harness_reads_a_catalogue_and_judges_what_it_finds() {
  let root = std::env::temp_dir().join(format!("xylogue-xsltconf-{}", std::process::id()));
...
fn the_harness_reads_the_layout_apaches_copy_uses() {
  let root = std::env::temp_dir().join(format!("xylogue-xsltconf-xalan-{}", std::process::id()));
  let _ = std::fs::remove_dir_all(&root);
