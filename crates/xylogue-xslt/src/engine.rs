...
impl Transform {
  ...
  /// [`Documents`](xylogue_xdm::Documents) handle with the model.
  ///
  /// # Errors
  ///
  ...
  /// use xylogue_xslt::{LoadedDocuments, Loader, Stylesheet, Transform};
  ///
...
impl<M: Model> Engine<'_, M> {
  ...
  fn run_node(&mut self, module: usize, node: NodeId, focus: Focus<M::Node>) -> Result<()> {
    ...
    match document.node_type(node) {
      NodeType::Text | NodeType::CdataSection => {
        ...
        if !text.chars().all(xylogue_core::chars::is_whitespace) {
          self.append_text(&text);
        }
        Ok(())
      }
  ...
  fn exsl_document(&mut self, module: usize, element: NodeId, focus: Focus<M::Node>) -> Result<()> {
    ...
    let href = xylogue_core::uri::resolve(&base, &href)?;
  ...
  fn literal_element(&mut self, module: usize, element: NodeId, focus: Focus<M::Node>) -> Result<()> {
    ...
    let attributes: Vec<(String, Option<String>, String)> = document
      ...
      .filter(|attribute| {
        document.namespace_uri(*attribute) != Some(xylogue_core::XMLNS_NS_URI)
  ...
  fn evaluate(
    ...
    xylogue_xpath::evaluate_in(&parsed, &context)
  }
  ...
  fn expression(&mut self, expression: &str) -> Result<Expr> {
    ...
    let parsed = xylogue_xpath::parse(expression)?;
  ...
  fn is_stripped(&self, node: M::Node) -> bool {
    ...
    if !self.model.string_value(node).chars().all(xylogue_core::chars::is_whitespace) {
      return false;
    }
    ...
    while let Some(node) = current {
      for attribute in self.model.attributes(node) {
        let is_xml_space = self.model.expanded_name(attribute).is_some_and(|name| {
          name.local == "space" && name.namespace.as_deref() == Some(xylogue_core::name::XML_NS_URI)
        });
        if is_xml_space {
...
mod tests {
  ...
  fn the_depth_guard_is_reached_before_the_stack_is() {
    ...
    let document = xylogue_dom::build::parse("<a/>".as_bytes()).expect("well-formed");
    let model = xylogue_xdm::DomModel::new(&document);
