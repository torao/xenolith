...
pub(crate) struct Running<N: Copy + Eq + std::hash::Hash> {
  ...
  adopted: RefCell<HashMap<xylogue_dom::NodeId, N>>,
...
impl<N: Copy + Eq + std::hash::Hash> Running<N> {
  ...
  pub(crate) fn adopted(&self, fragment: xylogue_dom::NodeId) -> Option<N> {
    self.adopted.borrow().get(&fragment).copied()
  }

  /// Remembers what a fragment was adopted as.
  pub(crate) fn remember_adopted(&self, fragment: xylogue_dom::NodeId, node: N) {
    self.adopted.borrow_mut().insert(fragment, node);
  }

  /// Puts a tree the transformation built into the model's node space.
  pub(crate) fn adopt(&self, document: xylogue_dom::Document, root: xylogue_dom::NodeId) -> Result<Option<N>> {
  ...
  functions
    .with("", "document", move |arguments: Vec<Value<M::Node>>, context: &Context<'_, M>| {
      ...
      for reference in wanted {
        let absolute = xylogue_core::uri::resolve(&base, &reference)?;
...
fn system_property<M: Model>(name: &str, context: &Context<'_, M>) -> Value<M::Node> {
  ...
  match local.as_str() {
    ...
    "vendor" => Value::String("xylogue".to_owned()),
    "vendor-url" => Value::String("https://github.com/torao/xylogue".to_owned()),
