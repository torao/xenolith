//! The properties xylograph is fuzzed against.
//!
    ...
    match reader.advance() {
      Ok(Some(kind)) => {
        ...
        match kind {
          ...
          EventKind::Text | EventKind::CData | EventKind::Comment => {
            let _ = parser.text();
          }
          _ => {}
        }
...
pub fn validate_document(data: &[u8]) {
  if let Ok(report) = xylograph_validate::validate(data) {
...
pub fn build_and_serialize(data: &[u8]) {
  ...
  let written = xylograph_serialize::Serializer::new().to_string(&document, root);
  ...
  let again = xylograph_serialize::Serializer::new().to_string(&reread, reread_root);
...
pub fn compile_expression(text: &str) {
  let Ok(expression) = xylograph_xpath::parse(text) else { return };
  let printed = expression.to_string();
  let reparsed = xylograph_xpath::parse(&printed)
