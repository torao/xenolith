//! What the `xylogue` binary does when it is actually run.
//!
...
impl Sandbox {
  fn new(name: &str) -> Self {
    let path = std::env::temp_dir().join(format!("xylogue-test-{name}-{}", std::process::id()));
...
fn xylogue(arguments: &[&str], stdin: &str) -> Output {
  let mut child = Command::new(env!("CARGO_BIN_EXE_xylogue"))
    .args(arguments)
    .stdin(Stdio::piped())
    .stdout(Stdio::piped())
    .stderr(Stdio::piped())
    .spawn()
    .expect("to start xylogue");
  child.stdin.as_mut().expect("a pipe to write to").write_all(stdin.as_bytes()).expect("to write to xylogue");
  child.wait_with_output().expect("to wait for xylogue")
}
...
fn a_stylesheet_runs_over_a_document_on_standard_input() {
  ...
  let output = xylogue(&["transform", stylesheet.to_str().unwrap()], DOCUMENT);
...
fn a_parameter_reaches_the_stylesheet() {
  ...
  let output =
    xylogue(&["transform", "--param", "greeting=Good day", stylesheet.to_str().unwrap(), input.to_str().unwrap()], "");
  assert!(output.status.success(), "{}", stderr(&output));
...
fn a_parameter_without_an_equals_sign_is_refused() {
  ...
  let output = xylogue(&["transform", "--param", "greeting", stylesheet.to_str().unwrap()], DOCUMENT);
...
fn the_result_can_be_written_to_a_file() {
  ...
  let output = xylogue(&["transform", "--output", result.to_str().unwrap(), stylesheet.to_str().unwrap()], DOCUMENT);
  assert!(output.status.success(), "{}", stderr(&output));
...
"#,
  ...
  // identifier.
  let output = xylogue(&["transform", main.to_str().unwrap()], DOCUMENT);
  assert!(output.status.success(), "{}", stderr(&output));
...
fn an_expression_prints_one_node_per_line() {
  let output = xylogue(&["xpath", "//item"], "<r><item>a</item><item>b</item></r>");
  assert!(output.status.success(), "{}", stderr(&output));
...
fn an_expression_that_is_not_a_node_set_prints_as_a_string() {
  let output = xylogue(&["xpath", "count(//item)"], "<r><item/><item/></r>");
  assert!(output.status.success(), "{}", stderr(&output));
...
fn a_namespace_binding_is_honoured() {
  let output = xylogue(&["xpath", "--namespace", "x=urn:d", "//x:a/text()"], "<r xmlns:d='urn:d'><d:a>found</d:a></r>");
  assert!(output.status.success(), "{}", stderr(&output));
...
fn selecting_nothing_is_only_a_failure_when_asked_for() {
  let quiet = xylogue(&["xpath", "//missing"], "<r/>");
  assert!(quiet.status.success());
  assert_eq!(stdout(&quiet), "");

  let strict = xylogue(&["xpath", "--fail-on-empty", "//missing"], "<r/>");
  assert_eq!(strict.status.code(), Some(1));
}

#[test]
fn an_expression_that_will_not_compile_is_a_failed_request() {
  let output = xylogue(&["xpath", "//["], "<r/>");
...
fn a_valid_document_and_an_invalid_one_are_told_apart() {
  ...
  let good = xylogue(&["validate", valid.to_str().unwrap()], "");
  assert!(good.status.success(), "{}", stdout(&good));
  assert!(stdout(&good).contains("valid"));

  let bad = xylogue(&["validate", invalid.to_str().unwrap()], "");
...
fn several_documents_are_all_reported() {
  ...
  let output = xylogue(&["validate", one.to_str().unwrap(), two.to_str().unwrap()], "");
...
fn a_document_is_written_out_indented() {
  let output = xylogue(&["format"], "<r><a><b>x</b></a></r>");
  assert!(output.status.success(), "{}", stderr(&output));
...
fn the_indent_width_is_the_one_asked_for() {
  let output = xylogue(&["format", "--indent", "4"], "<r><a/></r>");
  assert!(output.status.success(), "{}", stderr(&output));
  assert_eq!(stdout(&output), "<r>\n    <a/>\n</r>");
}
...
fn a_file_that_is_not_there_is_a_failed_request_not_a_no() {
  let output = xylogue(&["format", "no-such-file.xml"], "");
  assert_eq!(output.status.code(), Some(2));
...
fn xml_that_is_not_well_formed_is_a_failed_request() {
  let output = xylogue(&["format"], "<r><a></r>");
  assert_eq!(output.status.code(), Some(2));
