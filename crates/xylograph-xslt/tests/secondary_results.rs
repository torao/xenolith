...
use xylograph_xslt::{ResultSink, Stylesheet, Transform, transform};
...
impl ResultSink for Refuses {
  fn write(&mut self, _href: &str, _bytes: &[u8]) -> xylograph_core::Result<()> {
    Err(xylograph_core::Error::new(ErrorKind::Io, "this sink writes nothing".to_owned()))
  }
}
