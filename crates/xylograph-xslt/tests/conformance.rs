...
fn parse_loosely(xml: &str) -> Option<xylograph_dom::Document> {
  ...
  build::parse(format!("<xylograph-wrapper>{body}</xylograph-wrapper>").as_bytes()).ok()
}
...
fn canonical(document: &xylograph_dom::Document, node: xylograph_dom::NodeId, into: &mut String) {
  ...
  match document.node_type(node) {
    ...
    xylograph_dom::NodeType::Text | xylograph_dom::NodeType::CdataSection => {
    ...
    xylograph_dom::NodeType::Comment => {
      let _ = write!(into, "<!--{}-->", document.node_value(node).unwrap_or_default());
    }
    xylograph_dom::NodeType::ProcessingInstruction => {
...
fn expanded(document: &xylograph_dom::Document, node: xylograph_dom::NodeId) -> String {
...
fn is_namespace_declaration(document: &xylograph_dom::Document, attribute: xylograph_dom::NodeId) -> bool {
...
fn the_harness_reads_the_layout_apaches_copy_uses() {
  let root = std::env::temp_dir().join(format!("xylograph-xsltconf-xalan-{}", std::process::id()));
  let _ = std::fs::remove_dir_all(&root);
