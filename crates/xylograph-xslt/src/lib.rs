...
//! `xsl:copy-of` and [`AttributeSet`]s — as are [`Key`]s and the `key()` they serve. `xsl:sort`,
//! `xsl:number` and the output controls are still to come. A stylesheet can ask what is here
...
//!   [keys (§12.2)], the [additional functions of §12.4], and [what a stylesheet may ask about
//!   the processor (§15)].
