...
//! `xsl:sort`. `xsl:number` and the output controls are still to come. A stylesheet can ask what
//! is here before relying on it, with `element-available()` and `function-available()`. See
//! `ROADMAP.md`.
//!
//! # Features
//!
...
//!   [sorting (§10)], [keys (§12.2)], the [additional functions of §12.4], and [what a stylesheet
//!   may ask about the processor (§15)].
