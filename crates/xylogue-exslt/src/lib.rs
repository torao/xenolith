//! EXSLT extension functions for xylogue's XSLT 1.0 engine.
//!
...
//! the function, and the `xylogue` crate's behaviour report prints it.
//!
...
use xylogue_xpath::Functions;
use xylogue_xslt::{DocumentSource, NoDocuments};
...
/// that model cannot read — `xylogue_xslt::TreeSpace` is the usual one.
///
/// # Examples
///
...
/// use std::rc::Rc;
/// use xylogue_dom::build;
/// use xylogue_xdm::{DomModel, Documents, DomNode};
/// use xylogue_xpath::Functions;
/// use xylogue_xslt::{DocumentSource, TreeSpace};
///
...
/// let functions: Functions<DomModel<'_>> = xylogue_exslt::register_with(Functions::new(), &space);
