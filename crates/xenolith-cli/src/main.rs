//! `xenolith` — the command line of the library of the same name.
//!
...
#[derive(Debug, Parser)]
#[command(name = "xenolith", version, about = "Command-line XML tools", long_about = None)]
struct Cli {
  #[command(subcommand)]
  command: Command,
}
...
impl Loader for Files {
  fn load(&mut self, uri: &str) -> xenolith::Result<Vec<u8>> {
    ...
    fs::read(&path).map_err(|error| xenolith::Error::xslt(format!("{}: {error}", display(Path::new(&path)))))
  }
}
