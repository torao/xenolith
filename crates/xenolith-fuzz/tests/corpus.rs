...
fn every_document_parses_or_is_refused() {
  replay("documents", xenolith_fuzz::parse_document);
  replay("documents", xenolith_fuzz::parse_document_in_pieces);
  replay("documents", xenolith_fuzz::validate_document);
}

#[test]
fn every_document_that_builds_a_tree_can_be_written_and_read_back() {
  replay("documents", xenolith_fuzz::build_and_serialize);
}
...
fn the_properties_survive_what_is_not_xml_at_all() {
  ...
  for input in [
    ...
    xenolith_fuzz::parse_document(input);
