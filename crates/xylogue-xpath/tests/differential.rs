...
//!
//! Point `XYLOGUE_JAVA` at a `java` of version 11 or later to run it:
//!
//! ```text
...
//! single-file source mode, and reads expressions from its standard input. Without `XYLOGUE_JAVA`
...
use xylogue_xpath::XPathExpression;
...
fn java_answers(java: &PathBuf, xml: &str, expressions: &[String]) -> Result<Vec<Answer>, String> {
  let directory = std::env::temp_dir().join(format!("xylogue-differential-{}", std::process::id()));
...
fn the_same_expressions_give_the_same_answers_as_java() {
  let Some(java) = std::env::var_os("XYLOGUE_JAVA").map(PathBuf::from) else {
    eprintln!("skipped: set XYLOGUE_JAVA to a java of version 11 or later to compare against the JDK");
    return;
  };
